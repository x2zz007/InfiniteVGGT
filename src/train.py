...
from dust3r.datasets.seven_scenes import SevenScenesDataset 
...
def build_dataset(dataset, batch_size, num_workers, accelerator, test=False, fixed_length=False):
    ...
    # --- 在这里添加新的逻辑 ---
    # 检查配置文件中指定的 'name' 是否是 'seven_scenes'
    if dataset.name == 'seven_scenes':
        printer.info(f"Found 'seven_scenes' dataset. Building it manually.")
        # 根据 'test' 参数决定是加载 'train' 还是 'test' split
        split_str = 'test' if test else 'train'
        
        # 1. 实例化我们自定义的 SevenScenesDataset
        dset = SevenScenesDataset(root=dataset.path, split=split_str)
        
        # 2. 手动为这个 dataset 创建一个 DataLoader
